...
-- ============================================================
CREATE OR REPLACE TABLE PPIH_FULL_DB.ANALYTICS.IS_POS_TRANSACTION (
...
BEGIN
  ...
  WHILE (:v_i < 365) DO
    ...
    INSERT INTO PPIH_FULL_DB.ANALYTICS.IS_POS_TRANSACTION
    SELECT
...
CREATE OR REPLACE SEMANTIC VIEW PPIH_FULL_DB.ANALYTICS.DATACOMPASS_ANALYTICS
  TABLES (
    PPIH_FULL_DB.ANALYTICS.IS_POS_TRANSACTION,
  ...
  RELATIONSHIPS (
    TRADE_TO_STORES AS IS_POS_TRANSACTION(STORE_CODE) REFERENCES DATAMART_COMMON_STORES(STORE_CODE),
    TRADE_TO_ITEMS AS IS_POS_TRANSACTION(ITEM_CODE) REFERENCES DATAMART_COMMON_ITEMS(ITEM_CODE),
    TRADE_TO_MEMBERS AS IS_POS_TRANSACTION(MAJICA_NO) REFERENCES DATAMART_COMMON_MEMBERS(MAJICA_NO)
  )
  FACTS (
    IS_POS_TRANSACTION.ITEM_SALES_QUANTITY AS ITEM_SALES_QUANTITY,
    IS_POS_TRANSACTION.ITEM_SALES_AMOUNT AS ITEM_SALES_AMOUNT
  )
  DIMENSIONS (
    IS_POS_TRANSACTION.BUSINESS_DATE AS BUSINESS_DATE,
    IS_POS_TRANSACTION.STORE_CODE AS STORE_CODE,
    IS_POS_TRANSACTION.TRADE_KEY AS TRADE_KEY,
    IS_POS_TRANSACTION.MAJICA_NO AS MAJICA_NO,
    IS_POS_TRANSACTION.ITEM_CODE AS ITEM_CODE,
    IS_POS_TRANSACTION.TRADE_CLASS_3 AS TRADE_CLASS_3,
...
ALTER TABLE PPIH_FULL_DB.ANALYTICS.IS_POS_TRANSACTION
