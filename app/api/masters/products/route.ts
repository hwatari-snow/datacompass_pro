...
export async function GET() {
  ...
  try {
    const [md, major, middle, minor, makers] = await Promise.all([
      querySnowflake(`SELECT DISTINCT MD_CODE AS code, MD_NAME AS name FROM ${T_ITEMS} WHERE MD_CODE IS NOT NULL ORDER BY MD_CODE`),
      querySnowflake(`SELECT DISTINCT MAJOR_CODE AS code, MAJOR_NAME AS name, MD_CODE AS md_code FROM ${T_ITEMS} WHERE MAJOR_CODE IS NOT NULL ORDER BY MAJOR_CODE`),
      querySnowflake(`SELECT DISTINCT MIDDLE_CODE AS code, MIDDLE_NAME AS name, MAJOR_CODE AS major_code FROM ${T_ITEMS} WHERE MIDDLE_CODE IS NOT NULL ORDER BY MIDDLE_CODE`),
      querySnowflake(`SELECT DISTINCT MINOR_CODE AS code, MINOR_NAME AS name, MIDDLE_CODE AS middle_code FROM ${T_ITEMS} WHERE MINOR_CODE IS NOT NULL ORDER BY MINOR_CODE`),
      querySnowflake(`SELECT DISTINCT MAKER_CODE AS code, MAKER_NAME AS name FROM ${T_ITEMS} WHERE MAKER_CODE IS NOT NULL AND MAKER_NAME IS NOT NULL ORDER BY MAKER_NAME LIMIT 2000`),
    ])
